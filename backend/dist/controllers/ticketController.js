...
exports.getTicketSummary = exports.getTickets = exports.updateTicketStatus = exports.assignTicket = exports.getTicket = exports.getMyTickets = exports.createTicket = void 0;
...
const getMyTickets = async (req, res, next) => {
    try {
        const tickets = await Ticket_1.default.find({ requester: req.user._id }).populate('owner', 'name email');
...
const getTicket = async (req, res, next) => {
    try {
        const ticket = await Ticket_1.default.findById(req.params.id).populate('requester', 'name email').populate('owner', 'name email');
        if (!ticket) {
...
const assignTicket = async (req, res, next) => {
    try {
        ...
        await ticket.save();
        const updatedTicket = await Ticket_1.default.findById(req.params.id).populate('owner', 'name email');
...
const getTickets = async (req, res, next) => {
    try {
        ...
        const tickets = await Ticket_1.default.find(query).populate('requester', 'name email').populate('owner', 'name email');
        res.json({ success: true, message: 'Tickets fetched', data: tickets });
    }
    catch (error) {
        next(error);
...
const getTicketSummary = async (req, res, next) => {
    try {
        const summary = await Ticket_1.default.aggregate([
            {
                $group: {
                    _id: '$status',
                    count: { $sum: 1 },
